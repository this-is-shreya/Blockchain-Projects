...
function addRecords(string memory email, string memory link) public{

uint flag =0;
for(uint i=0;i<emails.length;i++){
if(keccak256(abi.encodePacked((emails[i]))) == keccak256(abi.encodePacked((email)))){

string memory str = strConcat(records[email],",",link,"","");
records[email] = str;
